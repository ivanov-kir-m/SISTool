...
namespace Novacode
{
    ...
    public class Paragraph : InsertBeforeOrAfter
    {
        ...
        private XElement GetParagraphNumberProperties()
        {
            ...
            if (numPrNode != null)
            {
                try
                {
                    var numIdNode = numPrNode.Descendants().FirstOrDefault(numId => numId.Name.LocalName == "numId");
                    if (numIdNode == null)
                    {
                        return null;
                    }
        ...
        public List<Hyperlink> Hyperlinks
        {
            get
            {
                ...
                List<XElement> hyperlinkElements =
                (
                ...
                foreach (XElement he in hyperlinkElements)
                {
                        ...
                        foreach (XElement r in e.ElementsAfterSelf(XName.Get("r", DocX.W.NamespaceName)))
                        {
                            ...
                            if (fldChar != null)
                            {
                                ...
                                if (fldCharType != null && fldCharType.Value.Equals("end", StringComparison.CurrentCultureIgnoreCase))
                                {
                                    try
                                    {
                                        Hyperlink h = new Hyperlink(Document, he, hyperlinkRuns);
                                        h.MainPart = MainPart;
                                        hyperlinks.Add(h);
                                    }
        ...
        private List<DocProperty> _docProperties;
        ...
        private float _indentationFirstLine;
        /// <summary>
        ...
        public float IndentationFirstLine
        {
            ...
            set
            {
                if (IndentationFirstLine != value)
                {
                    ...
                    string indentation = ((_indentationFirstLine / 0.1) * 57).ToString();
                    XAttribute firstLine = ind.Attribute(XName.Get("firstLine", DocX.W.NamespaceName));
                    if (firstLine != null)
        ...
        private float _indentationHanging;
        /// <summary>
        ...
        public float IndentationHanging
        {
            ...
            set
            {
                if (IndentationHanging != value)
                {
                    ...
                    string indentation = ((_indentationHanging / 0.1) * 57).ToString();
                    XAttribute hanging = ind.Attribute(XName.Get("hanging", DocX.W.NamespaceName));
                    if (hanging != null)
        ...
        private float _indentationBefore;
        /// <summary>
        ...
        public float IndentationBefore
        {
            ...
            set
            {
                ...
                {
                    _indentationBefore = value;

                    XElement pPr = GetOrCreate_pPr();
                    XElement ind = GetOrCreate_pPr_ind();

                    string indentation = ((_indentationBefore / 0.1) * 57).ToString(CultureInfo.GetCultureInfo("en-GB"));
        ...
        private float _indentationAfter;
        /// <summary>
        ...
        public float IndentationAfter
        {
            ...
            set
            {
                ...
                {
                    _indentationAfter = value;

                    XElement pPr = GetOrCreate_pPr();
                    XElement ind = GetOrCreate_pPr_ind();

                    string indentation = ((_indentationAfter / 0.1) * 57).ToString();
        ...
        public Paragraph InsertHyperlink(Hyperlink h, int index = 0)
        {
            ...
            Uri relsPath = new Uri(String.Format("/word/_rels/{0}.rels", path), UriKind.Relative);

            // Check to see if the rels file exists and create it if not.
            if (!Document.Package.PartExists(relsPath))
                HelperFunctions.CreateRelsPackagePart(Document, relsPath);
            ...
            else
            {
                ...
                else
                {
                    ...
                    hXml = (XElement)run.Xml.NextNode;
                }

                hXml.SetAttributeValue(DocX.R + "id", id);
            }
        ...
        public Alignment Alignment
        {
            ...
            set
            {
                ...
                if (_alignment != Novacode.Alignment.Left)
                {
                    if (jc == null)
                        pPr.Add(new XElement(XName.Get("jc", DocX.W.NamespaceName), new XAttribute(XName.Get("val", DocX.W.NamespaceName), _alignment.ToString())));
                    else
                        jc.Attribute(XName.Get("val", DocX.W.NamespaceName)).Value = _alignment.ToString();
                }
        ...
        public void InsertText(int index, string value, bool trackChanges = false, Formatting formatting = null)
        {
            ...
            DateTime insertDatetime = new DateTime(now.Year, now.Month, now.Day, now.Hour, now.Minute, 0, DateTimeKind.Utc);
            ...
            if (run == null)
            {
                ...
                if (trackChanges)
                    insert = CreateEdit(EditType.Ins, insertDatetime, insert);
                Xml.Add(insert);
            }
                    ...
                    case "ins":
                        {
                            ...
                            DateTime parentInsDate = DateTime.Parse(parentElement.Attribute(XName.Get("date", DocX.W.NamespaceName)).Value);
                            ...
                            */
                            if (trackChanges && parentInsDate.CompareTo(insertDatetime) == 0)
                            {
                                /*
                    ...
                    case "del":
                        {
                            ...
                            if (trackChanges)
                                insert = CreateEdit(EditType.Ins, insertDatetime, newRuns);
        ...
        public Paragraph AppendHyperlink(Hyperlink h)
        {
            // Convert the path of this mainPart to its equilivant rels file path.
            string path = MainPart.Uri.OriginalString.Replace("/word/", "");
            Uri relsPath = new Uri("/word/_rels/" + path + ".rels", UriKind.Relative);
        ...
        {
            string imageUriString = p.Img.Pr.TargetUri.OriginalString;
            ...
            {
                if (string.Equals(r.TargetUri.OriginalString, imageUriString, StringComparison.Ordinal))
                {
                    id = r.Id;
                    break;
                }
        ...
        public Paragraph InsertPicture(Picture p, int index = 0)
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    // The picture has to be the next element, extract it back out of the DOM.
                    pXml = (XElement)run.Xml.NextNode;
                }
            }
            // Extract the attribute id from the Pictures Xml.
            XAttribute aId =
            (
                from e in pXml.Descendants()
                where e.Name.LocalName.Equals("blip")
                select e.Attribute(XName.Get("embed", "http://schemas.openxmlformats.org/officeDocument/2006/relationships"))
            ).Single();
        ...
        public Table FollowingTable
        {
            get
            {
                return _followingTable;
            }
            internal set
            {
                _followingTable = value;
            }
        }
        ...
        public DocProperty InsertDocProperty(CustomProperty cp, bool trackChanges = false, Formatting f = null)
        {
            ...
            (
                ...
                new XAttribute(XName.Get("instr", DocX.W.NamespaceName), string.Format(@"DOCPROPERTY {0} \* MERGEFORMAT", cp.Name)),
                    new XElement(XName.Get("r", DocX.W.NamespaceName),
                        new XElement(XName.Get("t", DocX.W.NamespaceName), fXml, cp.Value))
            );
            ...
            if (trackChanges)
            {
                DateTime now = DateTime.Now;
                DateTime insertDatetime = new DateTime(now.Year, now.Month, now.Day, now.Hour, now.Minute, 0, DateTimeKind.Utc);
                e = CreateEdit(EditType.Ins, insertDatetime, e);
            }
		...
		public void RemoveText(int index, int count, bool trackChanges = false, bool removeEmptyParagraph=true)
        {
            // Timestamp to mark the start of insert
            DateTime now = DateTime.Now;
            DateTime removeDatetime = new DateTime(now.Year, now.Month, now.Day, now.Hour, now.Minute, 0, DateTimeKind.Utc);
                    ...
                    case "ins":
                        {
                            ...
                            object middle = CreateEdit(EditType.Del, removeDatetime, temp.Elements());
                    ...
                    default:
                        {
                            ...
                            object middle = CreateEdit(EditType.Del, removeDatetime, new List<XElement>() { Run.SplitRun(new Run(Document, splitRunBefore[1], run.StartIndex + GetElementTextLength(splitRunBefore[0])), min, EditType.Del)[0] });
                            processed += GetElementTextLength(middle as XElement);
    ...
    public class Run : DocXElement
    {
        ...
        private Dictionary<int, Text> _textLookup = new Dictionary<int, Text>();
        ...
        /// </summary>
        internal string Value { set { _text = value; } get { return _text; } }
            ...
            : base(document, xml)
        {
            ...
            foreach (XElement te in texts)
            {
                switch (te.Name.LocalName)
                {
                    case "tab":
                        {
                            _textLookup.Add(start + 1, new Text(Document, te, start));
                            _text += "\t";
                            start++;
                            break;
                        }
                    ...
                    case "delText":
                        {
                            ...
                            {
                                _textLookup.Add(start + te.Value.Length, new Text(Document, te, start));
                                _text += te.Value;
    ...
    internal class Text : DocXElement
    {
            ...
            : base(document, xml)
        {
            ...
            switch (Xml.Name.LocalName)
            {
                ...
                case "delText":
                    {
                        _endIndex = startIndex + xml.Value.Length;
                        _text = xml.Value;
                        break;
                    }
        ...
        {
            if (index < t._startIndex || index > t.EndIndex)
            ...
            if (t.Xml.Name.LocalName == "t" || t.Xml.Name.LocalName == "delText")
            {
                ...
                splitLeft = new XElement(t.Xml.Name, t.Xml.Attributes(), t.Xml.Value.Substring(0, index - t._startIndex));
                ...
                splitRight = new XElement(t.Xml.Name, t.Xml.Attributes(), t.Xml.Value.Substring(index - t._startIndex, t.Xml.Value.Length - (index - t._startIndex)));
