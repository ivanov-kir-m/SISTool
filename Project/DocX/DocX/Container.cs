...
namespace Novacode
{
    public abstract class Container : DocXElement
    {
        ...
        {
            #region Styles
            XDocument styleDocument;

            if (p.Styles.Count() > 0)
            {
                Uri stylePackageUri = new Uri("/word/styles.xml", UriKind.Relative);
                if (!Document.Package.PartExists(stylePackageUri))
                {
                    PackagePart stylePackage = Document.Package.CreatePart(stylePackageUri, "application/vnd.openxmlformats-officedocument.wordprocessingml.styles+xml", CompressionOption.Maximum);
                    using (TextWriter tw = new StreamWriter(new PackagePartStream(stylePackage.GetStream())))
                    {
                        styleDocument = new XDocument
                        (
                            new XDeclaration("1.0", "UTF-8", "yes"),
                            new XElement(XName.Get("styles", DocX.W.NamespaceName))
                        );

                        styleDocument.Save(tw);
                    }
                }

                PackagePart stylesDocument = Document.Package.GetPart(stylePackageUri);
                using (TextReader tr = new StreamReader(stylesDocument.GetStream()))
                {
                    styleDocument = XDocument.Load(tr);
                    XElement stylesElement = styleDocument.Element(XName.Get("styles", DocX.W.NamespaceName));

                    var ids = from d in stylesElement.Descendants(XName.Get("style", DocX.W.NamespaceName))
                ...
                using (TextWriter tw = new StreamWriter(new PackagePartStream(stylesDocument.GetStream())))
                    styleDocument.Save(tw);
            }
            #endregion
